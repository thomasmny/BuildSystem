...
public class SetProjectSubCommand implements SubCommand {
    ...
    public void execute(Player player, String worldName, String[] args) {
        ...
        if (args.length > 2) {
            plugin.getMessages().sendMessage(player, "worlds_setproject_usage");
            return;
        }

        if (buildWorld == null) {
            plugin.getMessages().sendMessage(player, "worlds_setproject_unknown_world");
            return;
        }
    ...
    public List<String> complete(Player player, String[] args) {
        ...
        return WorldsCompletions.permittedWorldNames(player, ws, "buildsystem.setproject", args[1]);
    }
