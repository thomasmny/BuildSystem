...
abstract class RegistryManagementMenu<T> extends PaginatedMenu {
    ...
    protected RegistryManagementMenu(BuildSystemPlugin plugin, Player player, String titleKey) {
        super(plugin.getMessages(), INVENTORY_SIZE, plugin.getMessages().getString(titleKey, player));
        this.plugin = plugin;
    }
    ...
    protected void populate(Player player) {
        ...
        registerPageItems(FIRST_CONTENT_SLOT, ITEMS_PER_PAGE, List.copyOf(entries()), this::createEntryButton);
        register(SLOT_BACK, createBackButton());
        register(SLOT_RESET, createResetButton());
        register(SLOT_ADD, createAddButton());
    ...
    private MenuButton createEntryButton(T entry) {
        return MenuButton.builder()
                .render((player, inventory, slot) -> {
                    String loreKey = isBuiltIn(entry) ? builtInLoreKey() : entryLoreKey();
                    icon(entry, player)
