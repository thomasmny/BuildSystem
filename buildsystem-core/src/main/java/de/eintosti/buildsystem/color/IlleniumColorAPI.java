...
public class IlleniumColorAPI {
    ...
    private static final java.util.List<ColorPattern> PATTERNS = Arrays.asList(
        new GradientPattern(), new SolidPattern(), new RainbowPattern());
    ...
    public static List<String> process(@Nonnull Collection<String> strings) {
        return strings.stream().map(IlleniumColorAPI::process).collect(Collectors.toList());
    }
    ...
    /**
     * Removes all color codes from the provided String, including IlleniumColorAPI
     * patterns.
     *
    ...
    private static ChatColor[] createGradient(@Nonnull Color start, @Nonnull Color end, int step) {
        ...
        for (int i = 0; i < step; i++) {
            Color color = new Color(
                start.getRed() + ((stepR * i) * direction[0]), start.getGreen() + ((stepG * i) * direction[1]),
