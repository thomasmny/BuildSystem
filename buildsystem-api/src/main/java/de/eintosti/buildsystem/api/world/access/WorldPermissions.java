...
/**
 * Manages and checks permissions related to {@link BuildWorld}s within the BuildSystem. This interface handles
 * permissions for actions such as entering, modifying, and executing commands in worlds.
 *
 * @since 3.0.0
 */
@NullMarked
public interface WorldPermissions {
     ...
     *
     * <p>A player can enter if any of the following conditions are met:
     *
     * <ul>
     *   <li>They have the administrative permission ({@link #hasAdminPermission(Player)}).
     *   <li>They can bypass the view permission ({@link #canBypassViewPermission(Player)}).
     *   <li>They are either the world's creator or an assigned builder.
     *   <li>The world is public (its permission is set to "{@code -}").
     *   <li>They possess the specific permission defined in the world's data.
     * </ul>
     *
     ...
     * </ul>
     *
     * <p>However, a player can bypass these restrictions if:
     *
     * <ul>
     *   <li>They have the administrative permission ({@link #hasAdminPermission(Player)}).
     *   <li>They are in a "build mode" that allows them to bypass building restrictions
     *       ({@link #canBypassBuildRestriction(Player)}).
     ...
     * world.
     *
     * <p>Permissions are handled as follows:
     *
     * <ul>
     *   <li>The world's creator can run the command if they have the base permission, optionally ending with
     *       {@code .self}.
     *   <li>All other players require the permission {@code <permission>.other} to execute the command.
     * </ul>
     *
    ...
    /**
     * Checks if the given {@link Player} possesses the administrative permission, typically
     * "{@code buildsystem.admin}". Players with this permission can bypass many world-specific restrictions.
     *
     * @param player The {@link Player} to check
    ...
    /**
     * Checks if the player can bypass the permission required to view a private world in the navigator. This is
     * separate from the `canEnter` permission and relates specifically to listing the world.
     *
     * @param player The {@link Player} to check
    ...
    /**
     * Checks if the given {@link Player} can bypass standard building restrictions due to being in a special "build
     * mode" or having a bypass permission. This allows players to modify worlds even if general building is disabled.
     *
     * @param player The {@link Player} to check
